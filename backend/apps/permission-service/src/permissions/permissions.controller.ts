...
export class PermissionsController {
  ...
  async listGrants(
    ...
    @Query('grantor_id') grantor_id?: string,
    ...
    if (caller?.role === 'EMPLOYEE' && grantor_id && grantor_id !== caller.userId) {
      throw new ForbiddenException('Employees may only list grants they issued');
    }
    return this.permissionService.listGrants(
      {
        actor_id: caller?.role === 'EMPLOYEE' && !grantor_id ? caller.userId : actor_id,
        grantor_id: caller?.role === 'EMPLOYEE' && grantor_id ? caller.userId : grantor_id,
        resource_type,
        resource_id,
        status,
        task_id,

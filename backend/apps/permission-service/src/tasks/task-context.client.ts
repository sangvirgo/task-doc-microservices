...
export class TaskContextClient {
  ...
  async getContext(taskId: string): Promise<PermissionTaskContext> {
    ...
    try {
      ...
      if (response.status === 404) throw new NotFoundException('Task not found');
      if (!response.ok) {
        throw new ServiceUnavailableException(`Task context lookup failed: ${response.status}`);
      }

      return (await response.json()) as PermissionTaskContext;
    } catch (error) {

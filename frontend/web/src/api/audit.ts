...
export const auditApi = {
  events: () => gatewayClient.get<AuditEventResponse[]>('/audit/events?limit=50').then((items) => items.map(({ id, event_type, occurred_at, resource_type, sequence_number }) => ({ id, event_type, occurred_at, resource_type, sequence_number }))),

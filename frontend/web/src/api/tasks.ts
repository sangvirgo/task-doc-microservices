...
const TASK_LIST_CACHE_TTL_MS = 5_000;
const taskListCache = new Map<string, { items: Task[]; expiresAt: number }>();
const taskListRequests = new Map<string, Promise<Task[]>>();
...
export const tasksApi = {
  list: (filters: Record<string, string> = {}) => {
    const path = `/tasks${query(filters)}`;
    const now = Date.now();
    const cached = taskListCache.get(path);
    if (cached && cached.expiresAt > now) return Promise.resolve(cached.items);
    const pending = taskListRequests.get(path);
    if (pending) return pending;

    const request = gatewayClient.getList<Task>(path)
      .then(items => {
        taskListCache.set(path, { items, expiresAt: Date.now() + TASK_LIST_CACHE_TTL_MS });
        return items;
      })
      .finally(() => {
        if (taskListRequests.get(path) === request) taskListRequests.delete(path);
      });
    taskListRequests.set(path, request);
    return request;
  },
  invalidateListCache: () => taskListCache.clear(),

...
export function AppShell({ session, children }: { session: SessionRecord; children: React.ReactNode }) {
  ...
  const navItems = session.role === 'ADMIN'
    ? [['Tổng quan', '/workspace', '◈'], ['Người dùng', '/admin/users', '♙'], ['Cảnh báo', '/admin/alerts', '◉'], ['Giám sát', '/admin/monitoring', '◈'], ['Hạ tầng', '/admin/infra', '▣'], ['Siêu dữ liệu kiểm toán', '/admin/audit', '▤']]
    : [['Tổng quan', '/workspace', '◈'], ['Công việc', '/tasks', '✓'], ['Tài liệu', '/documents', '▧'], ['Quyền tài liệu', '/grants', '⌘'], ['Nhật ký hoạt động', '/audit', '▤'], ['Thông báo', '/notifications', '◌']];

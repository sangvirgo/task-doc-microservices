...
export function LoadingState() { return <PageState title="Đang tải không gian làm việc">Preparing your session…</PageState>; }
...
export function ErrorState({ message, onRetry }: { message: string; onRetry?: () => void }) { return <PageState title="Đã xảy ra lỗi" action={onRetry ? <button onClick={onRetry}>Thử lại</button> : undefined}>{message}</PageState>; }

...
export function GrantList() {
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const formElement = event.currentTarget; const form = new FormData(formElement); const permissions = String(form.get('permissions')).split(',').map(value => value.trim()).filter(Boolean); setStatus('Creating grant…'); try { const created = await grantsApi.create({ grantor_id: userId, actor_id: String(form.get('actor_id')), resource_type: String(form.get('resource_type')), resource_id: String(form.get('resource_id')), task_id: String(form.get('task_id')), permissions, expires_at: new Date(String(form.get('expires_at'))).toISOString() }); setStatus(`Grant ${created.status.toLowerCase()} from the server.`); formElement.reset(); load(); } catch { setStatus('Grant was not accepted by the server.'); } };

...
export function RetentionPanel() {
  ...
  const load = () => {
    ...
    Promise.all([retentionApi.holds(), retentionApi.approvals(), documentsApi.list().catch(() => [])])
      .then(([holdItems, approvalItems, documentItems]) => { setHolds(holdItems); setApprovals(approvalItems); setDocuments(documentItems); })
  ...
  return <section><h1>Lưu giữ & hủy</h1><p className={styles.notice}>EMPLOYEE workspace only. Eligibility, approval and disposal are decided by the service; approval and execution require its DISPOSAL_APPROVE capability check.</p>
    <div className={styles.grid}><form className={styles.form} onSubmit={placeHold}><h2>Place retention hold</h2><label>Document ID<SearchableSelect name="document_id" required defaultValue=""><option value="" disabled>Choose a document</option>{documents.map(document => <option key={document.id} value={document.id}>{document.title} · {document.id.slice(0, 8)}</option>)}</SearchableSelect></label><label>Lý do<input name="reason" required /></label><button>Place hold</button></form>
    <form className={styles.form} onSubmit={approve}><h2>Approve disposal</h2><label>Document ID<SearchableSelect name="document_id" required defaultValue=""><option value="" disabled>Choose a document</option>{documents.map(document => <option key={document.id} value={document.id}>{document.title} · {document.id.slice(0, 8)}</option>)}</SearchableSelect></label><label>Lý do<input name="reason" required /></label><button>Approve disposal</button></form>
    <form className={styles.form} onSubmit={execute}><h2>Execute disposal</h2><label>Document ID<SearchableSelect name="document_id" required defaultValue=""><option value="" disabled>Choose a document</option>{documents.map(document => <option key={document.id} value={document.id}>{document.title} · {document.id.slice(0, 8)}</option>)}</SearchableSelect></label><button>Execute disposal</button></form>

...
export function TaskCollaboration({ taskId, members, mode = 'both' }: TaskCollaborationProps) {
  ...
  const composer = <form className={styles.composer} onSubmit={submitComment}><label htmlFor={`task-comment-${taskId}`}>Nội dung<textarea id={`task-comment-${taskId}`} name="content" required placeholder="Chia sẻ cập nhật, câu hỏi hoặc phản hồi…" /></label><div><span>Trao đổi được lưu lại cùng task.</span><button type="submit">Đăng bình luận</button></div></form>;
  ...
  return <section className={styles.section} aria-labelledby="task-collaboration-title">
    ...
    <section className={styles.thread} aria-label={activeTab === 'comments' ? 'Bình luận' : 'Dòng thời gian'}>{activeTab === 'comments' || mode === 'comments' ? <>{commentPanel}{composer}</> : activityPanel}<div ref={sentinelRef} data-testid="collaboration-load-more" className={styles.sentinel}>{current.hasNext && !current.loading ? <button type="button" onClick={loadMore}>Tải thêm</button> : null}</div></section>
  </section>;
}

...
export function TaskDocuments({ task, canUpload = false, members = [], participants = [], childTasks = [] }: { task: Task; canUpload?: boolean; members?: MemberOption[]; participants?: Participant[]; childTasks?: TaskChildSummary[] }) {
  ...
  const toggleSharePermission = (permission: string) => {
    ...
    if (next.includes('SHARE') && !hasContent) {
      setStatus(permission === 'SHARE'
        ? 'Quyền Chia sẻ (SHARE) phải đi kèm với Xem (PREVIEW) hoặc Tải xuống (DOWNLOAD). Vui lòng chọn thêm một trong hai quyền này.'
        : 'Không thể bỏ quyền Xem (PREVIEW) và Tải xuống (DOWNLOAD) trong khi vẫn còn quyền Chia sẻ (SHARE).');
      return;
    }
    setStatus('');
  ...
  const shareDocument = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (sharePermissions.includes('SHARE') && !sharePermissions.includes('PREVIEW') && !sharePermissions.includes('DOWNLOAD')) { setStatus('Quyền Chia sẻ (SHARE) phải đi kèm với Xem (PREVIEW) hoặc Tải xuống (DOWNLOAD). Vui lòng chọn thêm một trong hai quyền này.'); return; }
  ...
  return <section className={styles.section} aria-labelledby="task-documents-title">
    ...
    {loadFailed ? <div className={styles.loadError} role="alert"><span aria-hidden="true">!</span><div><strong>Không tải được tài liệu</strong><p>Đây là lỗi tải dữ liệu, không phải trạng thái “0 tệp”. Hãy thử tải lại.</p></div><button type="button" onClick={() => void load()}>Tải lại</button></div> : <div className={styles.documentGrid}>
      {items?.map(item => <article className={styles.documentCard} key={item.association_id}><span className={styles.documentIcon}>▧</span><div>{item.permissions.includes('PREVIEW') ? <button type="button" className={styles.documentTitle} onClick={() => openPreview(item)}>{item.title}</button> : <strong className={styles.documentTitleDisabled}>{item.title}</strong>}<small>{item.document_type} · v{item.current_version}</small></div>{item.permissions.includes("SHARE") && childTasks.length > 0 && <div className={styles.childShare}><label>Gắn vào sub-task<select value={childTaskId} onChange={event => setChildTaskId(event.target.value)}><option value="">Chọn sub-task</option>{childTasks.map(child => <option key={child.id} value={child.id}>{child.title}</option>)}</select></label><button type="button" disabled={!childTaskId || sharingToChild} onClick={() => void shareToChild(item)}>{sharingToChild ? "Đang gắn…" : "Gắn vào sub-task"}</button></div>}<footer className={styles.documentFooter} role="group" aria-label={`Thao tác với tệp ${item.title}`}><span>{item.permissions.join(' · ')}</span><div className={styles.documentActions}>{item.permissions.includes('PREVIEW') && <button type="button" onClick={() => openPreview(item)}>Xem trước</button>}{item.permissions.includes('DOWNLOAD') && <button type="button" className={styles.downloadButton} onClick={() => void downloadDocument(item)}>Tải xuống</button>}{item.permissions.includes('SHARE') && <button type="button" className={styles.shareButton} onClick={() => openShare(item)}>Chia sẻ quyền</button>}{canDetach(item) && <button type="button" className={styles.detachButton} disabled={detachingId === item.document_id} onClick={() => void detachDocument(item)}>Xóa khỏi task</button>}{!item.permissions.includes('PREVIEW') && !item.permissions.includes('DOWNLOAD') && <span className={styles.noAction}>Bạn không có quyền thao tác</span>}</div></footer></article>)}

...
export function TaskList() {
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setUploading(false);
    const successes = ['Đã tạo công việc', subtasks.length > 0 ? `${subtasks.length - subtaskFailures.length}/${subtasks.length} sub-task` : '', files.length > 0 ? `${files.length - uploadFailures.length}/${files.length} tài liệu` : ''].filter(Boolean).join(' · ');

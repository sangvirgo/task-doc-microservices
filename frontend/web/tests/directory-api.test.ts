...
afterEach(() => { adminApi.invalidateDirectory(); clearSession(); vi.unstubAllGlobals(); });
...
it('shares an in-flight directory request between concurrent callers', async () => {
  const fetchMock = vi.fn().mockResolvedValue(page([{ id: 'member', email: 'member@example.com' }], 1, false));
  vi.stubGlobal('fetch', fetchMock);

  const [first, second] = await Promise.all([adminApi.directory({ force: true }), adminApi.directory()]);

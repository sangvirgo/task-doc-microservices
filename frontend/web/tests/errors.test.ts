...
describe('Gateway error normalization', () => {
  ...
  it('preserves an authentication-service message for a locked account', async () => {
    const error = await toGatewayError(new Response(JSON.stringify({ message: 'Account is locked' }), { status: 401, headers: { 'content-type': 'application/json' } }));
    expect(error).toMatchObject({ status: 401, message: 'Account is locked' });
  ...
  it('formats structured validation issues instead of showing [object Object]', async () => {
    const error = await toGatewayError(new Response(JSON.stringify({ message: [{ path: ['task_id'], message: 'Required' }] }), { status: 400, headers: { 'content-type': 'application/json' } }));
    expect(error).toMatchObject({ status: 400, message: 'task_id: Required' });
  });  it.each([

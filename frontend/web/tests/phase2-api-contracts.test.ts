...
it('uses only typed Gateway task mutation paths and canonical lifecycle values', async () => {
  ...
  await tasksApi.create({ title: 'Task' }); await tasksApi.assign('task-id', 'employee-id'); await tasksApi.addParticipant('task-id', 'employee-id'); await tasksApi.status('task-id', 'IN_PROGRESS'); await tasksApi.block('task-id', 'Waiting'); await tasksApi.unblock('task-id'); await tasksApi.submit('task-id', 'Result'); await tasksApi.review('submission-id', 'APPROVED'); await tasksApi.review('task-id', 'submission-id', 'APPROVED', 'Đã kiểm tra');
  expect(fetchMock.mock.calls.map(call => call[0])).toEqual(['/gateway/tasks', '/gateway/tasks/task-id/assign', '/gateway/tasks/task-id/participants', '/gateway/tasks/task-id/status', '/gateway/tasks/task-id/block', '/gateway/tasks/task-id/unblock', '/gateway/tasks/task-id/submit', '/gateway/tasks/submissions/submission-id/review', '/gateway/tasks/task-id/submissions/submission-id/review']);
